...
class oddsNet():
   def __init__(self, name, train_data, test_data, val_data, scaler, architecture, learning_rate= .001, weight_decay=False, num_epochs=10, batch_size = 2048, pos_weight=1, sort_criteria='ev', iterations=1000):
      self.name = name
      ...
      if self.architecture == 'sigmoid':
         ...
         print(f'Size of first input layer: {train_data.tensors[0].shape[1]} nodes')
      ...
      self.val_loader = torch.utils.data.DataLoader(val_data, batch_size = self.batch_size, shuffle = True)
   ...
   def train(self):
      if self.architecture == 'catboost':
         ...
         val_df = pd.DataFrame(val_features)  # Replace column='' names with your actual feature names
   ...
   # We have fully trained our model. now we're finding the best thresholds to use for this model 
    ...
    # Gets data regarding the whole run 
      ...
      for threshold in thresholds:
        ...
        unscaled_true_pos = self.scaler.inverse_transform(true_pos[:, :49]) if len(true_pos) > 0 and hasattr(self.scaler, 'scale_') else np.array([])
   ...
   def backtest(self):
      ...
      self.apply_best_ev_model_to_validate()
