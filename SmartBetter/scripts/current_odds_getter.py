...
def make_list_of_live_games(data, params) -> dict:
    for game in data:
        ...
        try:
            for bookie in game['bookmakers']:
                ...
                if bookie['key'].upper() == "draftkings".upper() and check_if_live(commence_time_mst):
                    params['NBA TEAMS'].append(game['home_team'])
                    params['NBA TEAMS'].append(game['away_team'])
