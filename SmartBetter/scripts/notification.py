...
def notify_phone(string, output):
    message = f"{string} {output}"
...
def notify_model_thresh(team, prob, odds):
    prob_round = round(float(prob), 2)
    # Should implement a break even odds point for the bet based on the hit rate of the model 
    message = f'{team} {odds} {prob_round}'

    notify_phone("Got one: ", message)
